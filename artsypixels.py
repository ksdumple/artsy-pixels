...
import GridMod
from GridMod import colorPallet
from GridMod import pixelArt
from GridMod import menu
...
paintBrush = pygame.image.load("logo.png")
currentVersion = 1.1
...
def openFile(path):
    ...
    if f[-1] == str(currentVersion):
      ...
      window = Tk()
      window.withdraw()
      messagebox.showerror("Unsupported Version", "The file you have opened is not supported in this version.")
...
def initalize(cols, rows, showGrid=False):
   ...
   pygame.display.set_caption('Artsy Pixels')
   ...
   colorList = [(0,0,0), (255,255,255), (255,0,0), (0,0,255), (186,255,201), (255,179,186), (186,225,255), (255,255,186), (255,223,186)]
   pallet.setColor(colorList)

   tools = menu(win, 300, 40, 5, 1, True, grid.width - 310, grid.height + 50)
   tools.drawGrid()
   ...
   lineThickness = menu(win, 180, 40, 4, 1, True, grid.width - 250, grid.height + 10)
   lineThickness.drawGrid()

   buttons = ['1', '2', '3', '4']
   lineThickness.setText(buttons)

   saveMenu = menu(win, 140, 40, 2, 1, True, grid.width - 600, grid.height + 25)
   saveMenu.drawGrid()
...
window = Tk()
window.title('Artsy Pixels')

t_var = StringVar()
t_var.trace('w', updateLabel)

label = Label(window, text='# of Rows and Columns (e.g: 25,50): ')
    ...
    for event in ev:
        if event.type == pygame.QUIT:
            ...
            if pygame.display.get_caption()[0].count('*') > 0: 
               if messagebox.askyesno("Saying Goodbye?", "Would you like to save before closing?"):
                  if savedPath != "":
            ...
            try:
                ...
                if pos[1] >= grid.height: 
                    if pos[0] >= tools.startx and pos[0] <= tools.startx + tools.width and pos[1] >= tools.starty and pos[1] <+ tools.starty + tools.height: #If the mouse ic clicking on the tools grid
                        ...
                        if clicked.text == 'Draw': 
                            color = (0,0,0)
                        elif clicked.text == 'Erase':
                            color = (255,255,255)
                        elif clicked.text == 'Fill':
                            doFill = True
                        elif clicked.text == 'Replace':
                            replace = True
                        elif clicked.text == 'Clear':
                            grid.clearGrid()
                            tools.drawGrid() 
                            buttons = ['Draw', 'Erase', 'Fill', 'Replace', 'Clear']
                            tools.setText(buttons)
